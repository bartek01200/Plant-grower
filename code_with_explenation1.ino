//need to calculate water usage and compare waste water on average 
//weather and its affect on plant show analytics on thingsSpeak
...
const unsigned long HALF_DAY = 12UL * 60UL * 60UL * 1000UL; // 12 hours in ms
...
const float PUMP_FLOW_ML_PER_SEC = 55.56; // pump rate
unsigned long pumpOnStart   = 0;
float totalWaterDeliveredMl = 0.0;
const float manualWasteMl   = 2000.0; // assume 2L wasted manually

unsigned long monthStartMs = 0;
...
const unsigned long MONTH_DURATION = 30UL * 24UL * 60UL * 60UL * 1000UL; 
